...
AnnData2ACE <- function(file,
    ...
    counts.idx <- which(names(input_assays) == "counts")
    if (length(counts.idx) > 0) {
        perm <- c(counts.idx, setdiff(1:length(input_assays), counts.idx))
        input_assays <- input_assays[perm]
    }
    if ("counts" %in% names(input_assays)) {
        input_assays <- c(list(counts = input_assays[["counts"]]), input_assays[-1, drop = F])
    }
    invisible(gc())
        ...
        for (mn in names(obsm)) {
            ...
            if ("encoding-type" %in% names(attr)) {
                if (attr[["encoding-type"]] %in% c("csc_matrix", "csr_matrix")) {
                    ...
                    obj <- read.HD5DF(obsm, mn, compression_level = compression_level)
